...
#Carregar os pontos----
pontos_occ <- data.table::fread(here("Dados",
                              "Chiropotes.csv")) %>% 
  dplyr::tibble()
...
mapa <- sf::st_read(dsn ="Areas de endemismo ribas/areas_of_endemism.shp" ,
...
g
loc <- here("Imagens", "Mapa ocorrências Chiropotes teste.tiff") # para modificar o nome da pasta, 
ggsave(loc, g)

# Filtrar para os países que estão localizados

map <- mapa[mapa$End_Areas %in% c("Belém", "Xingú"), ] 

# Carregar interflúvios
eco <- terra::vect("Areas de endemismo ribas/areas_of_endemism.shp")
eco <- sf::st_as_sf(eco)
names(eco)

# garantir mesmo CRS do mapa
eco <- st_transform(eco, st_crs(map))
...
for (i in 1:n.groups) {
  ...
  # ===============================
  # Área de calibração por interflúvio
  # ===============================
  
  # pontos da espécie
  pts <- st_as_sf(
    pontos_occ.list3[g.limit, ],
    coords = c("longitude", "latitude"),
    crs = st_crs(map)
  )
  
  # descobrir quais interflúvios possuem registros
  join <- st_join(pts, eco)
  
  ids <- unique(join$End_Areas)
 
  print(ids)
 
  # selecionar os interflúvios utilizados
  xy.mcp[[i]] <- eco %>%
    dplyr::filter(End_Areas %in% ids)
  
  # unir todos em um único polígono
  xy.mcp[[i]] <- xy.mcp[[i]] %>%
    summarise()
...
g
loc <- here("Imagens", "Mapa Interfluvio Chiropotes.tiff")
ggsave(loc, g)

#Salvar os pontos de ocorrência numa tabela.csv
write.csv(pontos_occ.list3, 
          file = here("Dados", "ocorrências Chiropotes interfluvio.csv"),
          row.names = FALSE)
...
write.table(D,
            file = here::here("Tabelas", "D de Chiropotes interfluvios.txt"),
            sep = " ")
...
loc <- here("Imagens", "D de Schoener Chiropotes interfluvios.tiff") # para modificar o nome da pasta, 
...
loc <- here("Imagens", "Valor de p (teste sim) Chiropotes interfluvios.tiff") # para modificar o nome da pasta, 
ggsave(loc, Sim_heatmap) 



write.table(sim,
            file = here::here("Tabelas", "sim_Chiropotes interfluvios.txt"),
            sep = " ")
...
loc <- here("Imagens", "Equivalência_Chiropotes interfluvios.tiff") # para modificar o nome da pasta, 
ggsave(loc, Equi_heatmap) 



write.table(equi,
            file = here::here("Tabelas", "equi_Chiropotes interfluvios.txt"),
            sep = " ")
...
loc <- here("Imagens", "Unfilling_Chiropotes interfluvios.tiff") # para modificar o nome da pasta, 
...
write.table(unfilling,
            file = here::here("Tabelas", "unfilling_Chiropotes interfluvios.txt"),
            sep = " ")
...
loc <- here("Imagens", "Expansion_Chiropotes interfluvios.tiff") # para modificar o nome da pasta, 
ggsave(loc, Exp_heatmap) 



write.table(expansion,
            file = here::here("Tabelas", "expansion_Chiropotes interfluvios.txt"),
            sep = " ")
...
loc <- here("Imagens", "Stability_Chiropotes interfluvios.tiff") # para modificar o nome da pasta, 
ggsave(loc, Est_heatmap) 



write.table(stability,
            file = here::here("Tabelas", "stability_Chiropotes interfluvios.txt"),
            sep = " ")
...
for (i in 2:n.groups) {
  ...
  for (j in 1:(i - 1)) {
    ...
    arquivo <- here::here( "Imagens", paste0("Nicho_", Especie[j], "_x_", Especie[i],"Interfluvius", ".png"))
    ...
    Nich.din<- ecospat.plot.niche.dyn(z[[i]], z[[j]], 0, 
                                      ...
                                      name.axis1 = "PC1",
                                      name.axis2 = "PC2") + mtext(paste(Especie[j],"x", Especie[i]), font = 3)
    dev.off()
     }
}
...
Nicho.esp <- ecospat.plot.niche (z[[1]], 
                                 title = "Nicho Chiropotes Belém ",
                                 name.axis1 = "PC1",
                                 name.axis2 = "PC2",
                                 cor=FALSE)


Nicho.esp <- ecospat.plot.niche (z[[2]], 
                                 title = "Nicho Chiropotes Xingu",
                                 name.axis1 = "PC1",
                                 name.axis2 = "PC2",
                                 cor=FALSE)
